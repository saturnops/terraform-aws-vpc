locals {
  public_subnets                       = var.enable_public_subnet ? length(var.public_subnet_cidrs) > 0 ? var.public_subnet_cidrs : [for netnum in range(0, 2) : cidrsubnet(var.vpc_cidr, 8, netnum)] : []
  private_subnets                      = var.enable_private_subnet ? length(var.private_subnet_cidrs) > 0 ? var.private_subnet_cidrs : [for netnum in range(2, 4) : cidrsubnet(var.vpc_cidr, 3, netnum)] : []
  database_subnets                     = var.enable_database_subnet ? length(var.database_subnet_cidrs) > 0 ? var.database_subnet_cidrs : [for netnum in range(4, 6) : cidrsubnet(var.vpc_cidr, 8, netnum)] : []
  intra_subnets                        = var.enable_intra_subnet ? length(var.intra_subnet_cidrs) > 0 ? var.intra_subnet_cidrs : [for netnum in range(6, 8) : cidrsubnet(var.vpc_cidr, 8, netnum)] : []
